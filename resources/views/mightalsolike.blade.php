                ...
                <div class="shopping-item-new card">
                    ...
                    <div class="card-body card-attributes">
                        ...
                        <div class="rating">
                            ...
                            <br>
                            {{ $product->comments->count() }} <span>отзывов</span>
                        </div>

                        @if($product->quantity > 0)
                                    @if(!$product->branches->count())
                                    @if(!in_array($product->id, Cart::content()->pluck('id')->all()))
                                    <button class="btn text-center vkorzinu icon-color add-Cart" data-product="{{ $product->id }}" data-price="{{ $product->price }}" data-title="{{ $product->title }}" data-thumbnail="{{ $product->thumbnail }}">
                                    ...
                                    @else
                                        <a class="btn text-center vkorzinu icon-color active-new">
                                            Добавлена в корзину
                                        </a>
                                    @endif
                                    @else
