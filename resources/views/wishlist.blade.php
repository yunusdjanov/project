        ...
        <div class="container shop-area">
            ...
            <div class="shopping-items">
                @if($products->count() > 0)
                        ...
                        <div class="shopping-item card">
                            ...
                            <div class="card-body">
                                ...
                                <a href="{{ route('product.show', $product->id) }}"
                                   class="card-text"> {{ Str::limit($product->title, 25, '...') }} </a>
                                <div class="rating">
                                    ...
                                    @if(!$product->branches->count())
                                        @if(!in_array($product->id, Cart::content()->pluck('id')->all()))
                                            <div class="d-flex justify-content-between w-100">
                                                <button class="btn text-center icon-color addCart"
                                                        data-product="{{ $product->id }}" data-price="{{ $product->price }}"
                                                ...
                                                <form action="{{ route('wishlist.destroy' , $product->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
                                                    <button type="submit" class="wish-btn text-danger" style="position: absolute; bottom: 10px; right: 5px"><span
                                                            class="fas fa-heart" style="font-size: 24px"></span></button>
                                                </form>
                                            </div>

                                        @else
                                            <a class="btn text-center icon-color active-new" role="button">
                                                Добавлена в корзину
                                            </a>
                                        @endif
                                    @else
                                        <a class="btn text-center icon-color prosmotr-btn"
                    ...
                    @endforeach
                @else
                    <span>{{ 'Избранных товаров пока нет...' }}</span>
                @endif
